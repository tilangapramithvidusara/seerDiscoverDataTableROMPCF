...
export const generateProjectManagerMValue = async(inititlaData: any, analisisDesignPre: 
  {
    ...
    responseSubtotal: any}, condition: boolean, settingParameters?: any, isSnapshotModeEnable?: boolean, isCost?: boolean) => {
  ...
  try {
    ...
    if (percentData?.[projectManagementTypeValue] == percentData?.[100000001]) {
      ...
      let mValue = romParameter == "Hours" ? projectManagementValue :  projectManagementValue/hoursPerday 
      let msValue = romParameter == "Hours" ? projectManagementValue :  projectManagementValue/hoursPerday
      let mscValue = romParameter == "Hours" ? projectManagementValue :  projectManagementValue/hoursPerday
      ...
      if (isCost) {
        if (condition) {
          mValue = mValue * hoursPerday * (hasParameters ? hourlyRate : parameterModel[0]?.hourlyRate?.value)
          msValue = msValue * hoursPerday * (hasParameters ? hourlyRate : parameterModel[0]?.hourlyRate?.value)
          mscValue = mscValue * hoursPerday * (hasParameters ? hourlyRate : parameterModel[0]?.hourlyRate?.value)
        } else {
          mValue = mValue * (hasParameters ? hourlyRate : parameterModel[0]?.hourlyRate?.value)
          msValue = msValue * (hasParameters ? hourlyRate : parameterModel[0]?.hourlyRate?.value)
          mscValue = mscValue * (hasParameters ? hourlyRate : parameterModel[0]?.hourlyRate?.value)
        }
      }
      returnObject.projectManager.resultValue = mValue;
      returnObject.projectManager.resultValueMS = msValue;
      returnObject.projectManager.resultValueMSC = mscValue;
    } else if (percentData?.[projectManagementTypeValue] == percentData?.[100000000]) { // FTE
      let mValue = romParameter == "Hours" ? (projectManagementValue * h8) : (projectManagementValue * h8)/hoursPerday // if c2 === hours then get direct (parameterModel[0]?.designReview * h8)
      let msValue = romParameter == "Hours" ? (projectManagementValue * g8) : (projectManagementValue * g8)/hoursPerday
      let mscValue = romParameter == "Hours" ? (projectManagementValue * f8) : (projectManagementValue * f8)/hoursPerday
      // returnObject.projectManager.resultValue = romParameter == "Hours" ? projectManagementValue :  projectManagementValue/hoursPerday // if c2 === hours then get direct parameterModel[0]?.designReview
      // returnObject.projectManager.resultValueMS = romParameter == "Hours" ? projectManagementValue :  projectManagementValue/hoursPerday // if c2 === hours then get direct parameterModel[0]?.designReview
      // returnObject.projectManager.resultValueMSC = romParameter == "Hours" ? projectManagementValue :  projectManagementValue/hoursPerday
...
export const generateProjectManagerCostMValue = async(inititlaData: any, analisisDesignPre: 
  {
    // responseCustomRequirementDesign: any, responseAnalisisDesign: any, responseCustomisationDesign: any, responseIntegration: any, 
