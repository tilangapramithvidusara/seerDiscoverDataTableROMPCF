...
export const generateTestingMValue = async(inititlaData: any, analisisDesignPre: {responseCustomRequirementDesign: any, responseAnalisisDesign: any, responseCustomisationDesign: any, responseIntegration: any}, condition: boolean, isFte?: boolean, settingParameters?: any, isSnapshotModeEnable?: boolean) => {
  ...
  try {
    ...
    if (inititlaData) {
      let {hoursPerday, testingType, testing} = parameterModel[0];
      let testingValue = testing;
      let testingTypeValue = testingType;
      if (hasParameters) {
        ...
        testingValue = parseFloat(settingParameters?.formattedData[
          parameterKeyIndex.testing
        ]?.currentValue || '0')
        testingTypeValue = parseFloat(settingParameters?.formattedData[
          parameterKeyIndex.testing
        ]?.typeValueCurrent)
      }
      ...
      if (fte) {
        ...
        // if (hasParameters) {
        //   const testingValue = parseFloat(settingParameters?.formattedData[
        //     parameterKeyIndex.testing
        //   ]?.currentValue || '0')
        //   const testingTypeValue = parseFloat(settingParameters?.formattedData[
        //     parameterKeyIndex.testing
        //   ]?.typeValueCurrent)
